...
app.post("/api/forgot-password", async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ 
      email: oldUser.email, 
      id: oldUser._id.toString(), // Ensure ID is a string
      otp: otp 
    }, secret, {
      expiresIn: "5m",
    });
    ...
    var transporter = nodemailer.createTransport({
      service: "gmail",
      auth: {
        user: "kamalakar1625@gmail.com",
        pass: "pyfzkqrourkbdnix",
      },
    });
    ...
    transporter.sendMail(mailOptions, function (error, info) {
      if (error) {
        ...
        res.json({ 
          ...
          token: token,
          id: oldUser._id.toString() // Explicitly return the ID
        });
      }
    });
...
app.post("/api/verify-otp/:id/:token", async (req, res) => {
  ...
  try {
    ...
    res.json({
      status: "verified",
      email: verify.email,
      token: token, // Keep token for password reset
    });
  } catch (error) {
    console.log(error);
